...
/**
 * Created by dongxiaoxia on 2017/6/16.
 * <p/>
 * Spring断言工具类
 *
 * 做了一些小调整：去掉对StringUtils,CollectionUtils,ObjectUtils等的其他工具类的依赖
 */
...
//        if (StringUtils.hasLength(textToSearch) && StringUtils.hasLength(substring) &&
        if (hasLength1(textToSearch) && hasLength1(substring) &&
...
//        if (StringUtils.hasLength(msg)) {
    ...
    private static boolean hasLength1(String str){
        return (str != null && str.length() > 0);
    }

    private static boolean hasText1(String str) {
        if (!hasLength1(str)) {
            return false;
        }
